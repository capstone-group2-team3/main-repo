...
class ClinicalPatternScorer:
    def __init__(self, patterns_path: str = "data/clinical_patterns.json"):
    ...
    def _condition_matched(self, condition: dict, abnormal_labs: list[dict]) -> bool:
        """
        abnormal_labs: قائمة dicts، كل واحد شكله
        {"test_name": "Hemoglobin", "status": "Low", ...}
        (هاد الشكل يلي المفروض T10 يرجعه)

        condition: شكله {"lab": "Hemoglobin", "status": "Low"}
        أو {"lab": "TSH", "status": "Abnormal"} (أي شي مش طبيعي)
        """
    ...
    def score_patterns(self, abnormal_labs: list[dict]) -> list[dict]:
        results = []

        for pattern_code, pattern in self.patterns.items():
            conditions = pattern.get("conditions", [])
            logic = pattern.get("logic", "AND")  # لو ما فيه logic، الافتراضي AND
            ...
            if not triggered:
                continue  # هاد النمط ما تحقق، نستبعده
            ...
            if logic == "OR":
                ...
                confidence = "high"  # AND يعني لازم الكل يتحقق، فهو دايمًا high لو triggered

            evidence_for = [f"{c['lab']} {c['status']}" for c in matched]
            missing_evidence = [
                f"{c['lab']} {c['status']}" for c in conditions if c not in matched
            ]
